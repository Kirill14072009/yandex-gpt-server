...
app.post('/api/place-info', async (req, res) => {
  ...
  try {
    const prompt = `Ты — экскурсовод. Дай краткую историческую справку о достопримечательности: "${name}".

Координаты: ${latitude}, ${longitude}
${description ? `Описание из базы: ${description}` : ''}

ОПРЕДЕЛИ ТИП МЕСТА (одно-два слова): Памятник, Бюст, Мемориальная доска, Парк, Сквер, Музей, Собор, Церковь, Театр, Библиотека, Мост, Фонтан, Дворец, Усадьба, Крепость, Мемориал, Памятный знак.

Напиши 3-4 предложения О КОНКРЕТНОМ месте "${name}" по координатам ${latitude}, ${longitude}:

- Если памятник/бюст — напиши КОМУ посвящён, когда установлен, кто скульптор.
- Если музей — ЧЕМУ посвящён, что внутри.
- Если сквер/парк — в честь кого назван или просто "место для отдыха".
- Если собор/церковь — когда построен, архитектурный стиль.

❗ ВАЖНО: Это историческая справка для туристов. Отвечай нейтрально, как в учебнике истории. Не давай политических оценок. Просто факты.
